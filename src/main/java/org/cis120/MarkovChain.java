 ...
 * use null to mark the end of a sentence.
 * <p>
 ...
 * according to the current word's probability distribution. That successor
 * will be the new "current" word after the current one is returned by next().
 * <p>
...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        if (first == null) {
        	throw new IllegalArgumentException("invalid first word");
        } else {
     ...
     * <p>
     * If start is null, then hasNext() should return false. start need not
     * actually be a part of the chain (but it should have no successor). Note
     * that if start is a word that the MarkovChain has not been trained on,
     * calling next() on the MarkovChain afterwards will return null.
     *
     ...
     * 2. a successor picked from the probability distribution associated with
     * the most recently returned from next()
     *
    ...
    public String next() {
    	...
    	if (!hasNext()) {
    		throw new NoSuchElementException("No next words");
        }     	
        String x = currentWord;
       	if (chain.containsKey(currentWord)) {
      		
       		currentWord = chain.get(x).pick(ng); 
       	} else {
       		currentWord = null;
       	}
     ...
     *                  generate
     * @param pickFirst - whether or not to pick the first word in
     *                  {@code words} from {@code startWords}
     * @throws IllegalArgumentException if there are less than 1 words fed in
     */
    /** PUT IN MarkovChain.java */
    public void fixDistribution(List<String> words, boolean pickFirst) {
        ...
        if (startWords.count(curWord) < 1) {
            throw new IllegalArgumentException(
                    "first word " + curWord + " not present in " +
                            "startWords"
            );
        }
        ...
        while (words.size() > 0) {
            ...
            if (nextWord != null) {
                if (curDistribution.count(nextWord) < 1) {
                    throw new IllegalArgumentException(
                            "word " + nextWord + " not found as a child of"
                                    + " word " + curWord
                    );
                }
