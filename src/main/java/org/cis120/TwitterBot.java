...
public class TwitterBot {
     ...
     * training the MarkovChain with sentences sourced from that CSV file. Uses
     ...
     * @param numWords  - the number of words of each tweet
     ...
     *                  previous contents
     */
    public void writeTweetsToFile(int numTweets, int numWords, String filePath, boolean append) {
        writeStringsToFile(generateTweets(numTweets, numWords), filePath, append);
    }
     ...
     * <p>
     * 3.a If the MarkovChain has no more values in its Iterator but the
     * tweet is not yet at the required number of words, use
     * randomPunctuation() to end the sentence and then reset() to begin the
     * next sentence with a random start word.
     * <p>
     * Your tweet should be properly formatted with one space between each
     * word and between sentences. It should not contain any leading or
     * trailing whitespace. You should leave the words uncapitalized, just as
     * they are from TweetParser.
     * <p>
     * You should return an empty string if there were no sentences available
     * to train the Markov Chain or if the number of words is 0. You also
     * need to do some input validation to make sure the number of words is
     * not negative.
     *
     * @param numWords - The desired number of words of the tweet to be produced
    ...
    public String generateTweet(int numWords) {
        ...
        if (numWords < 0) {
        	throw new IllegalArgumentException("negative number of words");
        } else if (numWords == 0 || !mc.hasNext()) {
        	return result; 
        }
        if (mc.hasNext()) {
        	result += mc.next();
     ...
     * @param numWords  - the number of words that each generated tweet should have.
     * @return a List of Strings where each element is a tweet
     */
    public List<String> generateTweets(int numTweets, int numWords) {
        List<String> tweets = new ArrayList<String>();
        while (numTweets > 0) {
            tweets.add(generateTweet(numWords));
            numTweets--;
        }
        return tweets;
    }
    ...
    public static void main(String[] args) {
        ...
        List<String> tweets = t.generateTweets(10, 55);
...
//         uncommenting the following code:
         t.writeTweetsToFile(10, 55, PATH_TO_OUTPUT_TWEETS, false);
    }
