...
public class TweetParser {
    ...
    /**
     * Given a String that represents a line extracted from a CSV file and an
     * int that represents the column of the CSV file that we want to extract
     * from, return the contents of that column from the String. Columns in the
     * CSV file are zero indexed.
     * <p>
     ...
     * @param csvColumn - the column of the line whose contents ought to be
     *                  returned
    ...
    /**
     * Given the argument pathToCSVFile and the column that the tweets are in,
     * use the extractColumn and a FileLineIterator to extract every tweet from
     * the CSV. (Recall that extractColumn returns null if there is no data at
     * that column.) You should skip lines in the CSV for which the tweetColumn
     * is out of bounds.
     *
     * @param pathToCSVFile - a String representing a path to a CSV file
     *                      containing tweets
     * @param tweetColumn   - the number of the column in the CSV file that
     *                      contains the tweet
    ...
    static List<String> csvFileToTweets(String pathToCSVFile, int tweetColumn) {
        ...
        FileLineIterator x = new FileLineIterator(pathToCSVFile);
        List<String> list = new LinkedList<String>();
        
        while (x.hasNext()) {
        	String y = x.next();
        	String p = extractColumn(y, tweetColumn);
        	if (p != null) {
        		list.add(p);
        	}
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        String newTweet = removeURLs(tweet);
        List<String> x = tweetSplit(newTweet);
        
        
        for (String sentence: x) {
        	List<String> wordList = parseAndCleanSentence(sentence);
    ...
    public static List<List<String>> csvFileToTrainingData(String pathToCSVFile, int tweetColumn) {
        //the function already throws an error if theres no path to file.
    	
        List<String> x = csvFileToTweets(pathToCSVFile, tweetColumn);
        String result = "";
        for (String s : x) {
        	result += s; 
        }
    	...
    	return parseAndCleanTweet(result);
    }
